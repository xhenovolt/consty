...
export default function TaskLogsPage() {
  ...
  useEffect(() => {
    fetch("http://localhost/consty/api/task_logs.php")
      .then(res => res.json())
  ...
  return (
    <RequireAuth>
      <div className="w-full mx-auto py-8 px-2 md:px-0">
        <h1 className="text-3xl font-extrabold text-blue-700 dark:text-blue-300 mb-6">Task Logs</h1>
        {error && (
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tbody>
              {loading ? (
                ...
                logs.map(log => (
                  <tr key={log.id}>
                    <td className="px-6 py-4">{tasks.find(t => t.id === log.task_id)?.name || log.task_id}</td>
